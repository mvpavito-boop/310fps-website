...
const NAV = [
  ...
  { href: "/admin/pricing", label: "Каталог и цены", icon: Monitor },
...
export default function AdminLayout({
  ...
  useEffect(() => {
    fetch("/api/admin/leads?status=new&count=true")
      ...
      .then((data) => {
        if (data?.count) setNewLeadsCount(data.count);
      })
      .catch(() => {});
  }, [pathname]);
  ...
  return (
    <div className="min-h-screen bg-background pt-[88px] flex flex-col md:flex-row">
      <nav
        aria-label="Управление сайтом"
        className="flex flex-wrap gap-2 border-b border-line p-3 md:hidden"
      >
        {NAV.map((item) => (
          <Link
            key={item.href}
            href={item.href}
            className={cn(
              "min-h-11 rounded-lg px-3 py-3 text-xs",
              isActive(item) ? "bg-ember/15 text-flame" : "text-ash",
            )}
          >
            {item.label}
          </Link>
        ...
        <button
          ...
          className="min-h-11 px-3 text-xs text-ash"
        >
          Выйти
        </button>
      </nav>
      ...
      <aside
        className={cn(
          "flex-shrink-0 h-[calc(100vh-88px)] sticky top-[88px] hidden md:flex flex-col bg-card border-r border-text-secondary/10 transition-all duration-300 z-40",
          collapsed ? "w-16" : "w-56",
        )}
      >
        {/* Лого */}
