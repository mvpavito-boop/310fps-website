...
function StatCard({
  ...
  return (
    <Link
      href={href}
      className="group block bg-card border border-text-secondary/10 rounded-xl p-5 hover:border-accent-orange/30 transition-all duration-200"
    >
      <div className="flex items-start justify-between mb-4">
        ...
        >
          <Icon
            className={cn(
              "w-5 h-5",
              accent ? "text-white" : "text-accent-orange",
            )}
          />
        </div>
      ...
      <p className="text-2xl font-[family-name:var(--font-russo)] text-foreground">
        {value}
      </p>
      <p className="text-xs text-text-secondary font-sans mt-1">{label}</p>
      {sub && (
        <p className="text-xs font-sans mt-2 text-accent-orange">{sub}</p>
      )}
    </Link>
  );
}
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([commerce, leadsData, newLeadsData, reviews]) => {
        ...
        setStats({
          catalogCount: commerce.state?.draft.builds.length ?? 0,
          componentsCount: commerce.state?.draft.components.length ?? 0,
          leadsTotal: leadsData.total || 0,
          leadsNew: newLeadsData.count || 0,
          leadsToday,
          reviewsCount: Array.isArray(reviews)
            ? reviews.filter((r: ReviewSummary) => r.active).length
            : 0,
        });
      ...
      <div className="p-8">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="bg-card rounded-xl p-5 h-28 animate-pulse"
            />
          ))}
        </div>
      </div>
  ...
  return (
    <div className="p-6 lg:p-8 space-y-8">
      {/* Заголовок */}
      <div>
        <h1 className="text-2xl font-[family-name:var(--font-russo)] text-foreground uppercase">
          Дашборд
        </h1>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <div
            key={label}
            className="bg-card border border-text-secondary/10 rounded-xl p-4 flex items-center gap-3"
          >
            <Icon className={cn("w-5 h-5 shrink-0", color)} />
            <div>
              <p className="text-lg font-[family-name:var(--font-russo)] text-foreground">
                {value}
              </p>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <Link
            href="/admin/leads"
            className="text-xs text-accent-orange hover:underline font-sans"
          >
            Все заявки →
          </Link>
        </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-text-secondary/10">
                  <th className="text-left px-4 py-3 text-xs font-sans font-semibold text-text-secondary uppercase tracking-wider">
                    Дата
                  </th>
                  <th className="text-left px-4 py-3 text-xs font-sans font-semibold text-text-secondary uppercase tracking-wider">
                    Имя
                  </th>
                  <th className="text-left px-4 py-3 text-xs font-sans font-semibold text-text-secondary uppercase tracking-wider hidden md:table-cell">
                    Контакт
                  </th>
                  <th className="text-left px-4 py-3 text-xs font-sans font-semibold text-text-secondary uppercase tracking-wider">
                    Статус
                ...
                {recentLeads.map((lead, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-sm font-sans text-text-secondary hidden md:table-cell">
                        {lead.phone}
                      </td>
